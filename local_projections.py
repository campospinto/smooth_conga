...
class SplineProjectionOperators:
    ...
    def P_star_tilde_V(self, f, kind='ps'):
        """
        approximate on tilde spline space using the adjoint P operator with specified kind
        """
        assert kind in ['ms', 'db', 'ps']
        tilde_mass = self.mass_matrix_tilde_V
        f_moments = self.get_moments(f)
        P_matrix = self.proj_matrix_from_tV_to_V(kind)   #  get_smooth_proj_on_tilde_V(kind=kind)
...
def is_valid(space_kind=None, proj_kind=None):
    ...
    if proj_kind in ['L2', 'db', 'ms', 'ps']:
        return True
    # additional kinds
    if space_kind == 'V' and proj_kind in ['tL2+db', 'tL2+ms', 'tL2+ps']:
        return True
    return False
